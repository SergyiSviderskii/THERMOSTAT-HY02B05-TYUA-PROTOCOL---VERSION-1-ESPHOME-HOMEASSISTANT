    ...
    case TuyaCommandType::DATAPOINT_REPORT_SYNC:
      ...
      if (this->init_state_ == TuyaInitState::INIT_DATAPOINT) {
        if (this->protocol_version_ == 1) { 
          this->init_state_ = TuyaInitState::INIT_DATAPOINT_SET;  
          datapoint_id_1.reset(); 
        } else { 
      ...
      if (this->init_state_ == TuyaInitState::INIT_DATAPOINT_SET) {
        datapoint_id_1 = this->get_datapoint_(4);
        if (datapoint_id_1.has_value()) { 
          if (datapoint_id_1->value_enum == my_global_mode) {
            this->init_state_ = TuyaInitState::INIT_DATAPOINT_PROG;
            datapoint_id_1.reset(); 
            datapoint_id_2.reset(); 
          } else {
            this->set_timeout("datapoint_set", 1000, [this] { this->set_numeric_datapoint_value_(4, TuyaDatapointType::ENUM, my_global_mode, 1, false);});
            datapoint_id_1.reset();
          }
        } 
      }    
      ...
      if (this->init_state_ == TuyaInitState::INIT_DATAPOINT_PROG) { 
        if (datapoint_id_2.has_value()) { 
          this->set_timeout("datapoint_dump", 1000, [this] { this->dump_config(); });  
          this->init_state_ = TuyaInitState::INIT_DONE; 
        } else {
          if (datapoint_id_1.has_value()) {
            this->set_timeout("datapoint_prog", 1000, [this] { this->set_numeric_datapoint_value_(115, TuyaDatapointType::INTEGER, datapoint_id_1->value_int, 4, true);});    
            this->init_state_ = TuyaInitState::INIT_DATAPOINT_SET_PROG; 
            datapoint_id_1.reset(); 
          } else {
            datapoint_id_1 = this->get_datapoint_(115);
            if (datapoint_id_1.has_value()) {
              this->init_state_ = TuyaInitState::INIT_DATAPOINT_SET_PROG;
            } 
          }
      ...
      if (this->init_state_ == TuyaInitState::INIT_DATAPOINT_SET_PROG) {
        datapoint_id_2 = this->get_datapoint_(122);
        if (datapoint_id_2.has_value()) {   
          this->init_state_ = TuyaInitState::INIT_DATAPOINT_PROG;
          this->send_empty_command_(TuyaCommandType::DATAPOINT_QUERY);
        } else {
          if (buffer[0] == 115 && datapoint_id_1.has_value()) {
            this->init_state_ = TuyaInitState::INIT_DATAPOINT_PROG;
          }             
        }
      }  
...
void Tuya::send_raw_command_(TuyaCommand command) {
  if (this->init_state_== TuyaInitState::INIT_DONE && this->protocol_version_ == 1) {    
    optional<TuyaDatapoint> lock_1 = this->get_datapoint_(1);
    if (lock_1.has_value()) {
      optional<TuyaDatapoint> lock_2 = this->get_datapoint_(6);
      if (lock_2.has_value()) {
        if ((!lock_1->value_bool || (lock_1->value_bool && lock_2->value_bool)) && command.cmd == TuyaCommandType::DATAPOINT_DELIVER ) {
          if (!(command.payload[0] == 1 || command.payload[0] == 6))  {     
            command = TuyaCommand{.cmd = TuyaCommandType::DATAPOINT_QUERY, .payload = std::vector<uint8_t>{}};
          }    
        }   
      }
    }       
...
uint8_t Tuya::get_wifi_status_code_() {
  if (this->protocol_version_ == 0x01) {  
    uint8_t status = 0x01;
...
void Tuya::send_wifi_status_() {
  ...
  if (this->protocol_version_ == 0x01 && this->wifi_status_ == 0x03 && !remote_is_connected()) {
...
void Tuya::get_datapoint_addition (uint8_t datapoint_id, bool point_id) {
    if (!point_id) {
      datapoint_id_1.reset(); 
      datapoint_id_1 = this->get_datapoint_(datapoint_id);
    } else {  
      datapoint_id_2.reset(); 
      datapoint_id_2 = this->get_datapoint_(datapoint_id);
    }  
}
